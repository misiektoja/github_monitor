...
_REAL_WIZARD_VERIFY = monitor.wizard_verify_smtp
...
def test_smtp_rejection_keeps_password_private(monkeypatch, tmp_path, capsys, password, surface):
    ...
    try:
        if surface == "command":
            ...
            result = _REAL_WIZARD_VERIFY(settings, {"SMTP_PASSWORD": password})
        elif surface == "doctor":
            result = monitor.DoctorReport()
            monitor.doctor_check_notifications(result)
        else:
