...
def test_private_smtp_entry_preserves_significant_spaces(tmp_path, monkeypatch):
    ...
    with socketserver.TCPServer(("127.0.0.1", 0), Peer) as server:
        ...
        try:
            ...
            state = monitor.build_wizard_state(tmp_path / "settings.conf", tmp_path / "wizard.env")
            answers = iter(["y", "127.0.0.1", str(server.server_address[1]), "n", "fixture-user", "from@example.test", "to@example.test", ""])
            monitor.wizard_collect_email(state, input_func=lambda *args: next(answers), getpass_func=lambda _: password, stream=io.StringIO())
            assert state.secrets["SMTP_PASSWORD"] == password
            monitor.update_dotenv_file(state.dotenv_path, state.secrets)
            assert dotenv_values(state.dotenv_path)["SMTP_PASSWORD"] == password
        finally:
            server.shutdown()
            worker.join()
    assert attempts == [password, password, password]
...
def test_doctor_names_invalid_path_and_color_values(monkeypatch):
    ...
    monitor.doctor_check_output_paths(monitor.DoctorReport())
...
# Hides the unsaved GitHub token when a transport exception repeats its value
def test_private_github_token_is_redacted_from_debug_errors(monkeypatch, capsys):
    candidate = "synthetic-github-token-not-yet-saved"

    # Raises at the real HTTP adapter after checking the prepared authorization header
    def reject(adapter, request, **kwargs):
        assert request.headers["Authorization"] == "Bearer " + candidate
        raise requests.ConnectionError("Provider rejected " + candidate)

    monkeypatch.setattr(monitor, "DEBUG_MODE", True)
    monkeypatch.setattr(HTTPAdapter, "send", reject)
    with pytest.raises(monitor.GitHubTokenConfigurationError):
        monitor.validate_github_token(candidate)
