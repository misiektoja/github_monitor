...
def test_invalid_boolean_settings_are_reported_in_one_row(gm_module, monkeypatch):
    configure_healthy_doctor(gm_module, monkeypatch)
    monkeypatch.setattr(gm_module, "ERROR_NOTIFICATION", "false", raising=False)
    monkeypatch.setattr(gm_module, "SMTP_SSL", 1, raising=False)
    report = gm_module.DoctorReport(target_name="octocat")

    gm_module.doctor_check_configuration(report, doctor_args(), Mock())

    rows = [check for check in report.checks if check.label == "One or more on/off settings are invalid"]
    assert [check.status for check in rows] == ["FAIL"]
...
def test_a_numeric_on_off_setting_is_read_as_a_boolean(gm_module):
    parsed = gm_module.parse_config_content("VERIFY_SSL = 0\nDISABLE_LOGGING = 1\n")
...
def test_the_shipped_defaults_pass_the_boolean_check(gm_module):
    assert gm_module.runtime_boolean_errors() == []


# A malformed destination is a FAIL under the label every tool in the family uses, so a fix reads the same everywhere
def test_a_malformed_webhook_url_fails_under_the_family_label(gm_module, monkeypatch):
    configure_healthy_doctor(gm_module, monkeypatch)
    monkeypatch.setattr(gm_module, "WEBHOOK_ENABLED", True)
    monkeypatch.setattr(gm_module, "WEBHOOK_PROFILE_NOTIFICATION", True)
    monkeypatch.setattr(gm_module, "WEBHOOK_PROVIDER", "discord", raising=False)
    monkeypatch.setattr(gm_module, "WEBHOOK_URL", "discord.com/api/webhooks/1/abc", raising=False)
    report = gm_module.DoctorReport()
    ...
    assert "WEBHOOK_URL" in failure.advice.fix
