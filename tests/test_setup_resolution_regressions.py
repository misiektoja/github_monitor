...
def test_setup_keeps_saved_dotenv_destination(monkeypatch, tmp_path, override):
    ...
    monkeypatch.setattr(monitor, "wizard_choose_config_destination", lambda *args, **kwargs: config)
    monkeypatch.setattr(monitor, "wizard_collect_all", collect)
    with pytest.raises(Captured):
        monitor.run_setup_wizard(None, config_path=config, env_file=explicit if override else None, interactive=True)
