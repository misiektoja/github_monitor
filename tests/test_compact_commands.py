...
monitor = importlib.import_module("github_monitor")
...
def test_display_prefix_is_short(monkeypatch, host, executable, method):
    ...
    monkeypatch.setattr(sys, "argv", ["/private/long/tools/github_monitor.py" if method == "manual" else "/private/long/bin/github_monitor"])
    monkeypatch.setattr(monitor, "__file__", "/private/long/tools/github_monitor.py")
    ...
    expected = executable + " github_monitor.py --setup" if method == "manual" else "github_monitor --setup"
...
def test_help_does_not_display_runtime_paths():
    ...
    assert "github_monitor.py --setup" in result.stdout
