...
# Confirms every user-visible declaration reports the same newest version
def test_declared_versions_agree():
    ...
    release_notes = re.search(r"^# Changes in ([\d.]+) ", read_asset("RELEASE_NOTES.md"), re.M)
...
def test_local_hooks_run_the_pinned_linter():
    ...
    assert covered.match("github_monitor.py")
    assert covered.match("tests/test_repository_metadata.py")
    assert "github_monitor.py tests" in lint_command
