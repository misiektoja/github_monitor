...
def runtime_command(command, prefix=None):
    module = importlib.import_module("github_monitor")
    assert module.__file__ is not None
    manual = [sys.executable, str(Path(module.__file__).resolve())]
    packaged = [sys.executable, "-m", "github_monitor"]
    ...
    command = re.sub(r"(?<![\w/])python(?:3(?:\.\d+)?)? github_monitor\.py", lambda match: manual_prefix, command)
    return re.sub(r"(?<![\w/])github_monitor(?= |$)", lambda match: packaged_prefix, command)
